...
if [ -f "$SWARM_DIR/swarm.pem" ]; then
    ...
    while true; do
        ...
        if [ "$choice" == "1" ]; then
            ...
            cd $HOME && git clone https://github.com/ezlabs-nodes/rl-swarm/ > /dev/null 2>&1
    ...
    cd $HOME && [ -d rl-swarm ] && rm -rf rl-swarm; git https://github.com/ezlabs-nodes/rl-swarm/ > /dev/null 2>&1
fi
